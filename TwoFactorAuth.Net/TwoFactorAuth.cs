...
public class TwoFactorAuth
{
    ...
    public string CreateSecret() => CreateSecret(DEFAULTSECRETBITS, CryptoSecureRequirement.RequireSecure);
    ...
    public string GetCode(string secret) => GetCode(secret, GetTime());
    ...
    public string GetCode(string secret, DateTime dateTime) => GetCode(secret, DateTimeToTimestamp(dateTime));
    ...
    public bool VerifyCode(string secret, string code) => VerifyCode(secret, code, DEFAULTDISCREPANCY, out _);
    ...
    /// <returns>Returns true when the TOTP code is valid, false otherwise.</returns>
    public bool VerifyCode(string secret, string code, out long timeSlice) => VerifyCode(secret, code, DEFAULTDISCREPANCY, out timeSlice);
    ...
    /// <returns>Returns true when the TOTP code is valid, false otherwise.</returns>
    public bool VerifyCode(string secret, string code, int discrepancy) => VerifyCode(secret, code, discrepancy, GetTime(), out _);
    ...
    /// <param name="timeSlice">When this method returns, contains the timeslice that matched the code</param>
    /// <returns>Returns true when the TOTP code is valid, false otherwise.</returns>
    public bool VerifyCode(string secret, string code, int discrepancy, out long timeSlice) => VerifyCode(secret, code, discrepancy, GetTime(), out timeSlice);
    ...
    /// <returns>Returns true when the TOTP code is valid, false otherwise.</returns>
    public bool VerifyCode(string secret, string code, int discrepancy, DateTime dateTime) => VerifyCode(secret, code, discrepancy, DateTimeToTimestamp(dateTime), out _);
    ...
    /// <param name="timeSlice">When this method returns, contains the timeslice that matched the code</param>
    /// <returns>Returns true when the TOTP code is valid, false otherwise.</returns>
    public bool VerifyCode(string secret, string code, int discrepancy, DateTime dateTime, out long timeSlice) => VerifyCode(secret, code, discrepancy, DateTimeToTimestamp(dateTime), out timeSlice);
    ...
    /// </exception>
    public bool VerifyCode(string secret, string code, int discrepancy, long timestamp) => VerifyCode(secret, code, discrepancy, timestamp, out _);
    ...
    public string GetQrCodeImageAsDataUri(string label, string secret) => GetQrCodeImageAsDataUri(label, secret, DEFAULTQRCODESIZE);
    ...
    public string GetQrCodeImageAsDataUri(string label, string secret, int size) => size <= 0
    ...
    /// number of seconds.
    /// </exception>
    public void EnsureCorrectTime(int leniency = DEFAULTLENIENCY) => EnsureCorrectTime(new ITimeProvider[] {
            new NTPTimeProvider(),
    ...
    /// <summary>
    /// Calculates the timeslice (e.g. number of periods since <see cref="_epoch"/>) for a given timestamp and
    ...
    private long GetTimeSlice(long timestamp, int offset) => (timestamp / Period) + (offset * Period);

    /// <summary>
    /// Converts a <see cref="DateTime"/> to timestamp (based on UNIX EPOCH, see <see cref="_epoch"/>).
    /// </summary>
    ...
    private long DateTimeToTimestamp(DateTime value) => (long)(value.ToUniversalTime() - _epoch).TotalSeconds;

    private DateTime GetTime() => TimeProvider.GetTimeAsync().Result;
